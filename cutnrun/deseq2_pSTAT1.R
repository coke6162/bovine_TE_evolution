...
countdata <- read.csv(file.path(workingDir, "bamCounts_pSTAT1_merged_d100.tab"), sep = "\t", header = FALSE)
head(countdata)
...
# Read in bed file
all_mergedpeaks <- read.table(file.path(inDir, "all_pSTAT1_merged_mergedpeaks.bed"), sep="\t", header = FALSE)
...
write.table(resdata, file.path(workingDir, "pSTAT1_IFNG_2h_vs_UT.txt"), quote = FALSE, row.names = FALSE, sep = "\t")
write.table(resdata_padj, file.path(workingDir, "pSTAT1_IFNG_2h_vs_UT_padj.txt"), quote = FALSE, row.names = FALSE, sep = "\t")
write.table(resdata_baseMean100_padj0.90_log2FC0.05, file.path(workingDir, "pSTAT1_IFNG_2h_vs_UT_constitutive_baseMean100_padj0.90_log2FC0.05.txt"), quote = FALSE, row.names = FALSE, sep = "\t")

# Write bed output files
write.table(bed, file.path(workingDir, "pSTAT1_IFNG_2h_vs_UT.bed"), quote = FALSE, row.names = FALSE, col.names = FALSE, sep = "\t")
write.table(bed_padj, file.path(workingDir, "pSTAT1_IFNG_2h_vs_UT_padj.bed"), quote = FALSE, row.names = FALSE, col.names = FALSE, sep = "\t")
write.table(bed_constitutive_baseMean100_padj0.90_log2FC0.05, file.path(workingDir, "pSTAT1_IFNG_2h_vs_UT_constitutive_baseMean100_padj0.90_log2FC0.05.bed"), quote = FALSE, row.names = FALSE, col.names = FALSE, sep = "\t")