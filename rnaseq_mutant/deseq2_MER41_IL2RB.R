...
# This assumes that columns are in the same order as that specified in bam_order.txt
(genotype <- factor(c(rep("WT", 8), rep("KO", 6))))
...
write.table(resdata, file.path(workingDir, "MDBK_MER41_IL2RB_IFNG_vs_WT_IFNG.txt"), quote = FALSE, row.names = FALSE, sep = "\t")
write.table(resdata_LFC, file.path(workingDir, "MDBK_MER41_IL2RB_IFNG_vs_WT_IFNG_LFC_shrinkage_ashr.txt"), quote = FALSE, row.names = FALSE, sep = "\t")

# Write bed output files
write.table(bed, file.path(workingDir, "MDBK_MER41_IL2RB_IFNG_vs_WT_IFNG_tss.bed"), quote = FALSE, col.names = FALSE, row.names = FALSE, sep = "\t")
write.table(bed_LFC, file.path(workingDir, "MDBK_MER41_IL2RB_IFNG_vs_WT_IFNG_LFC_shrinkage_ashrtss.bed"), quote = FALSE, row.names = FALSE, col.names = FALSE, sep = "\t")