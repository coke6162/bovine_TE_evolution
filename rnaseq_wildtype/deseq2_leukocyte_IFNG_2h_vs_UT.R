...
(treatment <- factor(c(rep("UT", 3), rep("IFNG_2h", 3))))
...
resLFC <- lfcShrink(dds, coef="treatment_IFNG_2h_vs_UT", type="apeglm")
head(resLFC)
...
write.table(resdata, file.path(workingDir, "leukocyte_IFNG_2h_vs_UT.txt"), quote = FALSE, row.names = FALSE, sep = "\t")
write.table(resdataLFC, file.path(workingDir, "leukocyte_IFNG_2h_vs_UT_LFC_shrinkage_apeglm.txt"), quote = FALSE, row.names = FALSE, sep = "\t")
write.table(resdata_baseMean100_padj0.90_log2FC0.05, file.path(workingDir, "leukocyte_IFNG_2h_vs_UT_constitutive_baseMean100_padj0.90_log2FC0.05.txt"), quote = FALSE, row.names = FALSE, sep = "\t")

# Write bed output files
write.table(bed, file.path(workingDir, "leukocyte_IFNG_2h_vs_UT_tss.bed"), quote = FALSE, col.names = FALSE, row.names = FALSE, sep = "\t")
write.table(bed_constitutive_baseMean100_padj0.90_log2FC0.05, file.path(workingDir, "leukocyte_IFNG_2h_vs_UT_constitutive_baseMean100_padj0.90_log2FC0.05_tss.bed"), quote = FALSE, row.names = FALSE, col.names = FALSE, sep = "\t")